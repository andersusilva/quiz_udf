			...
			<?php require_once 'templates/ads.php';?>
     		<div class="row">
				<div class="col-xs-12 col-sm-8 col-md-8 col-lg-8">
					<h1 class="text-center text_underline"> Tempo Restante: <span id='timer'></span> </h1>
						...
						<?php foreach ($results['questions'] as $result) {
							...
							<?php
								 ...
								 if ( ( $remainder < 1 ) || ( $i == $number_question && $remainder == 1 ) ) {
								 	echo "<button id='".$j."' class='next btn btn-success' type='submit'>Finalizar</button>";
								 	echo "</div>";
								 }  elseif ( $rowcount > $number_question  ) {
								 	if ( $j == 1 && $i == $number_question ) {
										echo "<button id='".$j."' class='next btn btn-success' type='button'>Próximo</button>";
										echo "</div>";
										$i = 0;
										$j++;           
									} elseif ( $k == $rowcount ) { 
										echo " <button id='".$j."' class='previous btn btn-success' type='button'>Anterior</button>
													<button id='".$j."' class='next btn btn-success' type='submit'>Finalizar</button>";
										echo "</div>";
										$i = 0;
										$j++;
									} elseif ( $j > 1 && $i == $number_question ) {
										echo "<button id='".$j."' class='previous btn btn-success' type='button'>Anterior</button>
								                    <button id='".$j."' class='next btn btn-success' type='button' >Próximo</button>";
										echo "</div>";
										$i = 0;
										$j++;
									}
...
<script type="text/javascript">
    ...
    {
        return 'Suas perguntas respondidas são zeradas, por favor, selecione permanecer na página para continuar seu quiz';
    }
